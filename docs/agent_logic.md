...
route_after_detect ── len(candidates) > 1 ? ──┐
  ...
  │                                        │  VLM(gpt-5.5) 二次确认；输出 verified_result
  │                                            │
  ▼                                            ▼
[visualize]────────────────────────────────────────────────────────────────
...
### verify（仅多候选时触发）
- **Provider**：gpt-5.5（`verify.py:VLM_PROVIDER="gpt4o"`，推理模型做精度兜底）
- **输入**：`candidates`，`original_image_path`
- **过程**：对全部 present 候选（上限 `VERIFY_MAX=12`）逐个验证（不靠 Gemini confidence 排序，避免候选数 > K 时漏真 Waldo），每个：
  1. patch 内 bbox → 原图坐标（无精确 bbox 时退化为整个 patch）
  2. 向外扩展 30% padding（最小边长 120px），裁图保存到 `outputs/verify/`
  3. 发 `VERIFY_PROMPT` 二次确认，返回 `{"is_waldo": bool, "confidence": float, "reason": str}`
- **输出写入 State**：`candidates` 更新 `verified` / `verify_confidence` / `orig_bbox` / `verify_crop_path`；`verified_result` = `is_waldo=True` 中 `verify_confidence` 最高的 `orig_bbox`，无则 `None`
...
| verify | **gpt-5.5** | `VERIFY_PROMPT` | 这真的是 Waldo 吗 | 严格；多候选时去伪存真的精度兜底 |
...
gemini-3.5-flash present 二元信号最强（召回 94.4% / 误检 4.9%），但 confidence 失效（与 present 矛盾率 77%），故按 present 过滤、不靠 confidence 排序。大多数图只剩 1 个真候选 → 跳过 verify 省一次调用；多候选才走 verify（gpt-5.5）兜底精度。
