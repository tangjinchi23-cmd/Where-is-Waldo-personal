...
> **这是确定性 workflow，不是 LLM-driven agent。** 控制流由代码决定（线性流 + 一个 `len(candidates) > 1` 的确定性分支），VLM 只作分类器/比较器。2026-06-22：LangGraph 已移除，改为纯函数 + 生成器编排（`agent/pipeline.py`）。

## 1. 整体流程

`agent/pipeline.py::_run_nodes` 顺序调用各节点，每步 `state.update(delta)` 并 `yield (node, delta)`（第一次 VLM 调用推迟到 detect）：
...
[detect]   ───────────────────────────────────────────────────────────────
  ...
  ▼
if len(candidates) > 1 ?  （_run_nodes 里的确定性分支，非 LLM 决策）──┐
  │                                            │
  │ 否（单候选 / 空）                          │ 是（多候选）
  ▼                                            ▼
  │                                       [verify] ────────────────────────
  │                                        │  全部 present 候选从原图裁出（+30% padding），
...
> **设计沿革**：`evaluate → calibrate` 迭代回路于 2026-06-10 移除；`analyze` 节点于 2026-06-17 移除（VLM 推荐切割行列数优化了错误的变量——该切多大由 Waldo 绝对像素决定，切图前不可知，改为确定性算术）；LangGraph `StateGraph` 编排于 2026-06-22 移除（流程本质是 workflow，无需 agent 框架）。
...
- **过程**：对整图 `[0, 0, W, H]` 调 `vision/segment.py::tile_region(region, TILE_SIZE, image_size, TILE_OVERLAP)`：
...
- **输出写入 State**：`candidates`（**重置**），每项 `{patch_bbox, row, col, confidence=0.0, crop_path=None, verified=False}`
...
- **过程**：
  ...
  3. 并发调用 VLM（`MAX_CONCURRENT=10`，`ThreadPoolExecutor`），发送 `DETECT_PROMPT`，返回 `{"present": bool, "confidence": float}`
  4. **按 `has_waldo`（present）过滤**；保持行优先 patch 顺序，不按 confidence 排序（Gemini confidence 与 present 矛盾率 77%，无判别意义）
- **限流重试**：429 指数退避（15→30→60→120s，最多 4 次）

### 路由分支（agent/pipeline.py::_run_nodes）
- `if len(state["candidates"]) > 1:` → 跑 `verify`；否则（单候选 / 空）跳过、直接 `visualize`
- 这是**确定性代码分支，不是 LLM 决策**。依据：Gemini detect 高精度，大多数图每张只标 1 个真候选；少数冒 false positive 的图才需 verify 去伪存真
...
| `MAX_CONCURRENT` | `nodes/detect.py` | 10 | detect 并发数（付费 Tier 1 ~300 RPM） |
